...
void avl_insert(struct avl_tree* avl, int key) {
    assert(avl);
    avl->root = _avl_subtree_insert(avl->root, key);  // if the root has been rotated, we need the new root
}
...
struct avl_node *rightRotate(struct avl_node *N){
	...
	updateHeight(C);	
	if (C->parent)
		updateHeight(C->parent); // updating parent height
   return C;
}
...
struct avl_node* _avl_subtree_insert(struct avl_node* node, int key){
	...
	if (node == NULL)  // root was NULL
		return(_avl_node_create(key));
 
  	if (key <= node->key){  // insert left
		if (node->left == NULL){  // free spot
...
/*
 * Rebalance a subtree if necessary
 *
 */ 
...
struct avl_node* rebalance(struct avl_node* node) {
	...
	int child_bal;
	if (bal <= -2) {  // left heavy
		child_bal = getBalance(node->left);
		if (child_bal >= 1)  // double rotation
			leftRotate(node->left);
		return rightRotate(node);
	} else if (bal >= 2) {  // right heavy
		child_bal = getBalance(node->right);
		if (child_bal <= -1)  // double rotation
