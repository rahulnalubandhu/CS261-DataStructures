...
void dynarray_insert(struct dynarray* da, void* val) {
   ...
   */
    ...
    /* Strategy 1 - trying to change the resize capacity 
    This will be usefull for considering the bigger data to be stored like if we want to store
     more data then we can rezise it by increasing the cpacity*/
    //_dynarray_resize(da, 4 * da->capacity);

    /*Strategy 2 - changing the resize capcity to 8*/
    //_dynarray_resize(da, 8 * da->capacity);

    /* Strategy 3 - changing the if-condition by resizing if the size if 75% full.*/
    //if (da->size >= (3.0/4.0*(da->capacity))) {
    //_dynarray_resize(da, 2 * da->capacity);}
