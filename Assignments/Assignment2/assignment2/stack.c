...
void stack_free(struct stack* stack) {
    //if stack is not null then free the stack->list
    if(stack != NULL){
        if(stack->list != NULL){
...
int stack_isempty(struct stack* stack) {
    ...
    // created a function call check_empty to check  whether the list->head is empty or not
    if(check_empty(stack->list) == NULL)
        return 1;
    else
        return 0;
}
...
void* stack_top(struct stack* stack) {
	...
	// passing the list to top function in list.c
    if(stack->list != NULL){
        return top(stack->list);
...
void* stack_pop(struct stack* stack) {
    ...
    // passing the stack to the remove_node function to pop the head
    void* val = stack_top(stack);
	remove_node(stack->list);
  	return val;
}
